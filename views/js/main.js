...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // All pizzas have the same size at any given time (small, medium or large). There's no need to re-calculate dx and newwidth in the loop
...
function updatePositions() {
  ...
  var phaseArray = [];
  // Produces 5 unique numbers, and push them into an array
  for (var i = 0; i < 5; i++) {
    phaseArray.push(Math.sin(scrollTopCalculation + i) * 100);
  }
  // Places pizza images on screen
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Reduced the pizza number from 200 to 30
  var elem;
  for (var i = 0; i < 30; i++) {
